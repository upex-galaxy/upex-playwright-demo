...
test.describe('GX3-5059 ToolsQA | Elements | Web Table (CRUD) ', () => {
	...
	test('TC1: Validar agregar un registro en la tabla', async () => {
		...
		await test.step('Navegar a la página de Web Tables', async () => {
			await webTablesPage.goto();
		});

		await test.step('Hacer clic en el botón Agregar', async () => {
			await webTablesPage.clickOpenRegistrationFormButton();
		});

		await test.step('Llenar el formulario con datos válidos', async () => {
			await webTablesPage.fillRegistrationForm({ firstName, lastName, email, age, salary, department });
		});

		await test.step('Hacer clic en el botón Enviar', async () => {
			await webTablesPage.clickSubmitRegistrationFormButton();
		});

		await test.step('Verificar que el nuevo registro esté en la cuarta fila', async () => {
			const nameInFourthRow = await webTablesPage.getNameFromFourthRowFirstColumn();
			expect(nameInFourthRow).toBe(firstName);
		});
	});
	...
	test('TC2: Validar la acción del botón "Cancelar"', async () => {
		...
		await test.step('Hacer clic en el botón Cancelar', async () => {
			await webTablesPage.clickCloseFormButton();
		});
	...
	test('TC3: Validar ingresar caracteres especiales en el campo "First Name"', async ({ page }) => {
		...
		await test.step('Llenar el formulario con caracteres especiales en el primer nombre', async () => {
			await webTablesPage.fillRegistrationForm({ firstName: specialCharFirstName, lastName, email, age, salary, department });
		});
		...
		await test.step('Verificar que el nuevo registro con caracteres especiales esté presente', async () => {
			await webTablesPage.verifyNewRecordInForm(specialCharFirstName);
		});
	});

	test('TC4: Validar la edición de un registro existente', async ({ page }) => {
		const webTablesPage = new WebTablesPage(page);
		const { firstName, lastName, email, age, salary, department } = data.records[0];
		...
		await test.step('Editar el registro recién agregado en la fila 4', async () => {
			await webTablesPage.clickEditButtonForRecord(4);
		});

		// Estos son datos del formulario editados, colocados aqui solamente para probar el TC4
		...
		await test.step('Verificar que el registro actualizado esté presente', async () => {
			const recordRow = page.getByRole('gridcell', { name: 'UpdatedName' });
			await expect(recordRow).toBeVisible();
		});
	});

	test('TC5: Validar dejar el campo "First Name" vacío con otros campos válidos del formulario', async ({ page }) => {
		const webTablesPage = new WebTablesPage(page);
		const { lastName, email, age, salary, department } = data.records[0];
	...
	test('TC6: Validar la ordenación de registros por nombre', async ({ page }) => {
		const webTablesPage = new WebTablesPage(page);
	...
	test('TC7: Validar la ordenación de registros por apellido', async ({ page }) => {
		const webTablesPage = new WebTablesPage(page);

		await test.step('Ordenar por apellido', async () => {
			await webTablesPage.sortTableByColumnHeader('lastName');
		});

		await test.step('Obtener los apellidos y verificar ordenación', async () => {
			const lastNames = await webTablesPage.getAllColumnValues('lastName');
			await webTablesPage.verifyColumnValuesAreSorted(lastNames, false);
		});
	});

	test('TC8: Validar la ordenación de registros por salario', async ({ page }) => {
		const webTablesPage = new WebTablesPage(page);

		await webTablesPage.goto();

		await test.step('Ordenar por salario', async () => {
			await webTablesPage.sortTableByColumnHeader('salary');
		});

		await test.step('Obtener los salarios y verificar ordenación', async () => {
			const salaries = await webTablesPage.getAllColumnValues('salary');
			await webTablesPage.verifyColumnValuesAreSorted(salaries, true);
		});
	});

	test('TC9: Validar la paginación entre registros usando el botón "Siguiente"', async ({ page }) => {
		const webTablesPage = new WebTablesPage(page);
		...
		await test.step('Configurar el número de filas por página', async () => {
			await webTablesPage.setRowsPerPageAndGetRowCount('5');
		});

		await test.step('Obtener el número de filas en la página inicial', async () => {
			const initialRowCount = await webTablesPage.setRowsPerPageAndGetRowCount('5');
		...
		await test.step('Obtener el número de filas después de hacer clic en "Siguiente"', async () => {
			const rowCountAfterNext = await webTablesPage.setRowsPerPageAndGetRowCount('5');
			expect(rowCountAfterNext).toBeLessThanOrEqual(5);
		});
	});

	test('TC10: Validar la paginación entre registros usando el botón "Anterior"', async ({ page }) => {
		const webTablesPage = new WebTablesPage(page);
		...
		await test.step('Obtener el número de filas después de hacer clic en "Anterior"', async () => {
			const rowCountAfterPrevious = await webTablesPage.setRowsPerPageAndGetRowCount('5');
			expect(rowCountAfterPrevious).toBe(5);
		});
	});

	test('TC11: Validar la navegación a una página específica usando el campo de número de página', async ({ page }) => {
		const webTablesPage = new WebTablesPage(page);
		...
		await test.step('Ir a una página específica', async () => {
			await webTablesPage.goToPageNumber('2');
		});

		await test.step('Obtener el número de filas después de la navegación', async () => {
			const rowCountAfterNavigation = await webTablesPage.setRowsPerPageAndGetRowCount('5');
